...
impl Runnable<Arc<Mutex<Database>>> for LPush {
    ...
    /// to the head of the list, from the leftmost element to the rightmost element. So for
    /// instance the command LPUSH mylist a b c will result into a list containing c as first
...
pub mod test_lpush {
    ...
    #[test]
    fn test_01_lpush_values_on_an_existing_list() {
    ...
    #[test]
    fn test_02_lpush_values_on_a_non_existing_list() {
        let (notifier, _log_rcv, _cmd_rcv) = create_notifier();
