...
pub mod redis_messages {
    ...
    pub fn arguments_invalid_to(item: &str) -> MessageRedis  {
        MessageRedis{prefix: "ERR".to_string(), message: "wrong number of arguments for ".to_owned() + "\'" + item + "\'"+ " command"}
    }
     
    pub fn not_empty_values_for(item: &str) -> MessageRedis  {
        MessageRedis{prefix: "ERR".to_string(), message: "\'".to_owned() + item + "\'"+ " does not accept empty values"}
    }
    ...
    pub fn wrongtype_in_get_key() -> MessageRedis  {
        MessageRedis{prefix: "WRONGTYPE".to_string(), message: "Operation against a key holding the wrong kind of value".to_string()}
    }
    ...
    pub fn command_not_found_in(buffer: String) -> MessageRedis {
        ...
        MessageRedis{prefix: "ERR".to_string(), message: format!("unknown command \'{}\', with args beginning with: {}", command, args_received)}
    }
}
