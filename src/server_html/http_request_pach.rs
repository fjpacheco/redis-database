...
mod tests {
    ...
    #[test]
    fn test_read_http() {
        let emuled_request: String = String::from("GET /greeting HTTP/1.1\r\nHost: localhost:3000\r\nUser-Agent: curl/7.64.1\r\nAccept: */*\r\n\r\n");
        let mut headers_expected = HashMap::new();
        headers_expected.insert("Host".into(), " localhost".into());
        headers_expected.insert("Accept".into(), " */*".into());
        headers_expected.insert("User-Agent".into(), " curl/7.64.1".into());

        let request_parsed: HttpRequest = emuled_request.into();

        assert_eq!(Method::Get, request_parsed.method);
        assert_eq!(Version::V1_1, request_parsed.version);
        assert_eq!(Resource::Path("/greeting".to_string()), request_parsed.resource);
